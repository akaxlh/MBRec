...
class Recommender:
	...
	def run(self):
		...
		for ep in range(stloc, args.epoch):
			...
			if test:
				...
				if bestRes is None or reses['HR'] > bestRes['HR']:
					bestRes = reses
	...
	def sampleTestBatch(self, batchIds):
		...
		for i in range(batch):
			...
			rdnNegSet = np.random.permutation(negset)[:negNum]
			locset = np.concatenate((rdnNegSet, np.array([posloc])))
			tstLocs[i] = locset
			for j in range(negNum + 1):
	...
	def testEpoch(self):
		...
		for i in range(steps):
			...
			hit, ndcg = self.calcRes(np.reshape(preds, [ed-st, args.negNum+1]), temTst, tstLocs)
			epochHit += hit
			epochNdcg += ndcg
